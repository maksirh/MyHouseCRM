...
!function(st, lt, rt, t) {
    ...
    st.fn.simpleLightbox = function(N) {
        ...
        var h, t, P = "ontouchstart"in lt, Z = (lt.navigator.pointerEnabled || lt.navigator.msPointerEnabled,
        0), z = 0, H = st(), i = function() {
        ...
        }, L = !1, p = [], j = N.rel && !1 !== N.rel ? (t = N.rel,
        st(this).filter(function() {
            return st(this).attr("rel") === t
        })) : this, a = j.get()[0].tagName, d = (i = i(),
        0), K = !1 !== i, n = "pushState"in history, u = !1, o = lt.location, U = function() {
            return o.hash.substring(1)
        }, $ = U(), f = function() {
            U();
            var t = "pid=" + (J + 1)
              , e = o.href.split("#")[0] + "#" + t;
            n ? history[u ? "replaceState" : "pushState"]("", rt.title, e) : u ? o.replace(e) : o.hash = t,
            u = !0
        }, B = function(e, a) {
            var n;
            return function() {
        ...
        }, Q = "simplelb", e = st("<div>").addClass("sl-overlay"), s = st("<button>").addClass("sl-close").html(N.closeText), g = st("<div>").addClass("sl-spinner").html("<div></div>"), G = st("<div>").addClass("sl-navigation").html('<button class="sl-prev">' + N.navText[0] + '</button><button class="sl-next">' + N.navText[1] + "</button>"), l = st("<div>").addClass("sl-counter").html('<span class="sl-current"></span>/<span class="sl-total"></span>'), m = !1, J = 0, v = 0, r = st("<div>").addClass("sl-caption " + N.captionClass + " pos-" + N.captionPosition), V = st("<div>").addClass("sl-image"), c = st("<div>").addClass("sl-wrapper").addClass(N.className), x = function(t) {
            ...
            m = !0,
            J = j.index(t),
            H = st("<img/>").hide().attr("src", t.attr(N.sourceAttr)).attr("data-scale", 1).attr("data-translate-x", 0).attr("data-translate-y", 0),
            ...
            if (H.length) {
                ...
                st(l).on("error", function(t) {
                    ...
                    g.hide();
                    var e = 1 == s || -1 == s;
                    v === J && e ? ot() : (N.alertError && alert(N.alertErrorMessage),
                    it(e ? s : 1))
                }),
                l.onload = function() {
                    void 0 !== s && j.eq(J).trigger(st.Event("changed.simplelightbox")).trigger(st.Event((1 === s ? "nextDone" : "prevDone") + ".simplelightbox")),
                    ...
                    1 == s || -1 == s) {
                        ...
                        N.animationSlide && (K ? (tt(0, 100 * s + "px"),
                        setTimeout(function() {
                            tt(N.animationSpeed / 1e3, "0px")
                        }, 50)) : o.left = parseInt(st(".sl-image").css("left")) + 100 * s + "px"),
            ...
            H.data("scale", t),
            H.data("translate-x", e),
            H.data("translate-y", a)
        }, y = function() {
            st(lt).on("resize." + Q, _),
            st(rt).on("click." + Q + " touchstart." + Q, ".sl-close", function(t) {
                t.preventDefault(),
                L && ot()
            }),
            ...
            G.on("click." + Q, "button", B(function(t) {
                t.preventDefault(),
                Z = 0,
                it(st(this).hasClass("sl-next") ? 1 : -1)
            }, N.throttleInterval));
            var e, a, n, i, o, s, l, r, c, h, p, d, u, f, g, m, v, x, b, y, C, w, T, E, S, M, I, k = 0, q = 0, X = 0, D = 0, Y = !1, A = !1, O = 0, R = !1, W = at(1, 1, N.maxZoom), F = !1;
            V.on("touchstart." + Q + " mousedown." + Q, function(t) {
                ...
                if ("mousedown" == (t = t.originalEvent).type)
                    ...
                    1 === I) {
                        if (F)
                            return H.addClass("sl-transition"),
                            Y = Y ? (nt(0, 0, W = 1),
                            et("0px", "0px", W),
                            !1) : (W = N.doubleTapZoom,
                            nt(0, 0, W),
                ...
                return !!A || (K && (O = parseInt(V.css("left"))),
                A = !0,
                z = Z = 0,
                ...
                "touchmove" == (t = t.originalEvent).type) {
                    ...
                    null === u && (u = M),
                    1 <= Math.abs(u - M) && (y = at(M / u * W, 1, N.maxZoom),
                    T = (s * y - a) / 2,
                    E = (o * y - e) / 2,
                    S = y - W,
                    x = s * y <= a ? 0 : at(l - (C - n - a / 2 - l) / (y - S) * S, -1 * T, T),
                    b = o * y <= e ? 0 : at(r - (w - i - e / 2 - r) / (y - S) * S, -1 * E, E),
                    et(x + "px", b + "px", y),
                    1 < y && (Y = !0,
                    st(".sl-caption").fadeOut(200)),
                    u = M,
                ...
                Z = k - q,
                z = X - D,
                N.animationSlide && (K ? tt(0, -Z + "px") : V.css("left", O - Z + "px")))
                ...
                P && "touchend" == t.type && (0 === (I = t.touches.length) ? (nt(W, x, b),
                1 == W && (Y = !1,
                ...
                A) {
                    ...
                    Math.abs(Z) > N.swipeTolerance && e ? it(0 < Z ? 1 : -1) : N.animationSlide && (K ? tt(N.animationSpeed / 1e3, "0px") : V.animate({
                        left: O + "px"
                    }, N.animationSpeed / 2)),
                    N.swipeClose && 50 < Math.abs(z) && Math.abs(Z) < N.swipeTolerance && ot()
                }
            ...
            st("<img />").attr("src", j.eq(t).attr(N.sourceAttr)).on("load", function() {
                -1 == p.indexOf(st(this).attr("src")) && p.push(st(this).attr("src")),
            ...
            if (!(m || (t < 0 || t >= j.length) && !1 === N.loop)) {
                ...
                N.animationSlide && (K ? tt(N.animationSpeed / 1e3, -100 * e - Z + "px") : a.left = parseInt(st(".sl-image").css("left")) + -100 * e + "px"),
            ...
            if (!m) {
                ...
                N.history && (n ? history.pushState("", rt.title, o.pathname + o.search) : o.hash = "",
                clearTimeout(h)),
        ...
        return N.close && s.appendTo(c),
        N.showCounter && 1 < j.length && (l.appendTo(c),
        l.find(".sl-total").text(j.length)),
        N.nav && G.appendTo(c),
